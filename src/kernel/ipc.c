...
PUBLIC int sys_call(
        ...
        Message *msg_ptr  /* 消息指针 */
){
    register Process *caller;
    Message *msg_phys, *msg_vir;
    int rs;
...
//           caller->logic_nr , op, src_dest_msgp, msg_ptr);
    ...
    /*==============================*/
    if(op == IN_OUTBOX) {
        msg_vir = (Message *) src_dest_msgp;
        if(msg_vir != NIL_MESSAGE)
            caller->inbox = (Message *) proc_vir2phys(caller, msg_vir);
        if(msg_ptr != NIL_MESSAGE)
            caller->outbox = (Message *) proc_vir2phys(caller, msg_ptr);
        return OK;
    }
...
//        assert(caller->logic_nr != src_dest_msgp);
        ...
        if(msg_ptr == NIL_MESSAGE)
            msg_phys = (Message *) caller->outbox;
        else
            msg_phys = (Message *) proc_vir2phys(caller, msg_ptr);
    ...
    if( !(caller->flags & SENDING) ) {          /* 首先，我自己不能处于发送消息的状态中 */
        ...
        for(sender = waiters[caller->logic_nr];
            ...
            if(sender->logic_nr == src || src == ANY) {
                /* 拷贝消息 */
                msg_copy( (phys_addr) sender->transfer, (phys_addr) msg_phys );
